...
bugged_units_fixer = {
    ...
    "Redcap": lambda unit: fix_onhit_summon(unit, RedMushboom, "Fire Seed"),
    "Slime Brewer": lambda unit: fix_onhit_summon(unit, GreenSlime, "Slime Shot"),
}
...
def modify_class(cls):
    ...
    if cls is SearingSealSpell:
        ...
        def get_description(self):
            return ("Gain Seal of Searing.\n"
                    ...
                    "When the seal expires, it deals [fire] damage to all enemies in line of sight equal to its number of charges divided by 4.\n"
    ...
    if cls is SplittingBuff:
        ...
        def on_death(self, evt):
            self.owner.level.queue_spell(summon_children(self))
    ...
    if cls is MonsterChainLightning:
    ...
    if cls is GiantFireBombSuicide:
    ...
    if cls is AngelSong:
        ...
        def cast_instant(self, x, y):
            for unit in self.caster.level.get_units_in_ball(Point(x, y), self.get_stat('radius')):
                if unit.is_player_controlled:
                    continue
                if (Tags.Living in unit.tags or Tags.Holy in unit.tags):
                    unit.deal_damage(-self.get_stat('heal'), Tags.Heal, self)
    ...
    if cls is Thorns:
        ...
        def on_init(self):
            self.owner_triggers[EventOnDamaged] = self.on_spell

        def on_spell(self, evt):
            if not isinstance(evt.source, Spell) or not (isinstance(evt.source, LeapAttack) or evt.source.melee):
                return
    ...
    if cls is SpiderSpawning:
        ...
        def on_death(self, evt):
            ...
            if not are_hostile(self.owner, evt.unit):
                return
            self.owner.level.queue_spell(summon_spider(self, evt.unit))
    ...
    if cls is TombstoneSummon:
        ...
        def cast_instant(self, x, y):
            ...
            if p:
                ...
                ghost.source = self.owner.source
                apply_minion_bonuses(self.owner.source, ghost)
    ...
    if cls is SpawnOnDeath:
        ...
        def on_death(self, evt):
            self.owner.level.queue_spell(summon_units(self))
        ...
        def on_death(self, evt):
            ...
            if evt.damage_event:
                ...
                if not self.unit.is_player_controlled and not self.unit.name == "Mordred":
                    RiftWizard.main_view.queue_effect(s)
                else:
                    # Clear all effects on the square except last death
                    RiftWizard.main_view.effects = [e for e in RiftWizard.main_view.effects if not ((e.x == self.unit.x) and (e.y == self.unit.y))]
                    RiftWizard.main_view.effects.append(s)
            ...
            if self.unit.is_player_controlled:
                ...
                if self.unit.is_boss:
                    RiftWizard.main_view.play_sound("death_boss")
                else:
                    RiftWizard.main_view.play_sound("death_enemy")

            SteamAdapter.unlock_bestiary(self.unit.name)
            if self.unit.parent:
                SteamAdapter.unlock_bestiary(self.unit.parent.name)

    if cls is Moonspeaker:
    ...
    if cls is LightningBoltSpell:
        ...
        def fmt_dict(self):
            stats = Spell.fmt_dict(self)
            stats["max_channel"] = self.get_stat("max_channel", base=10)
            return stats
    ...
    if cls is Dominate:
        ...
        def get_description(self):
            return ("Target enemy unit with [{hp_threshold}_max_HP:heal] or lower becomes your minion.").format(**self.fmt_dict())
    ...
    if cls is ShieldSiphon:
        ...
        def on_init(self):
            ...
            self.upgrades['shield_burn'] = (5, 2, "Shield Burn", "Deal [fire] damage to each enemy equal to 5 times the number of [SH:shields] stolen.")
    ...
    if cls is IceTap:
        ...
        def on_spell_cast(self, evt):
            ...
            self.owner.level.queue_spell(reset(self))
    ...
    if cls is DeathGazeSpell:
        ...
        def bolt(self, source, target):
            ...
            if dealt and self.get_stat("vampiric"):
                source.deal_damage(-dealt, Tags.Heal, self)
            yield False

    if cls is WizardSwap:
